...
class ModelPaymentPayfortFort extends Model {
	public function getMethod($address, $total) {
		$this->language->load('payment/payfort_fort');
            ...
            if ($sadad_enabled){
                $extra_options .= '$("label[for=payfort_fort]").closest("tr").after(\'<tr class="highlight"><td><input type="radio" name="payment_method" value="payfort_fort" id="payfort_sadad"></td><td><label for="payfort_sadad">SADAD</label></td></tr>\"\')';
            }
            // if ($naps_enabled){
                // $extra_options .= ';$("label[for=payfort_fort]").closest("tr").after(\'<tr class="highlight"><td><input type="radio" name="payment_method" value="payfort_fort" id="payfort_naps"></td><td><label for="payfort_naps">NAPS</label></td></tr>\"\')';
            // }
